...
# Try a faulty signup
$ua1->submit_form();
$ua1->content_contains('You must enter a e-mail address',
    'E-mail should be required');
$ua1->content_contains('You must enter a username',
    'Username should be required');
$ua1->content_contains('You must enter a password',
...
$ua1->content_contains('id="email" value="' . $data{fields}{email} . '"',
    'Check pre-filled form');
$ua1->submit_form(
    fields => {
...
my $user =
  Maximus->model('DB::User')->find({username => $data{fields}{username}});
...
$ua1->submit_form(fields => {username => $data{fields}{username},});
$ua1->content_contains('<h1>Confirmation e-mail sent</h1>', 'h1 check');
$ua1->content_contains('check your e-mail for instructions',
...
ok( request(
    ...
    'Faulty reset password link'
);

$ua1->get_ok(
    '/account/reset_password/' . $data{fields}{username} . '/' . $hash,
...
is($user->password, sha1_hex($data{fields}{password}),
